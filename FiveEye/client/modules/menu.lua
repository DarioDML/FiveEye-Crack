        ...
        if isusingfuncs then
            ...
            if master then
                ...
                for bb, aZ in ipairs(GetActivePlayers()) do
                    local bc = {r = 0, g = 255, b = 189, a = 255}
...
-- MENU 2
local menu2 = MenuV:CreateMenu(false, "FiveEye Admin Tools", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Admin Menu')
...
local espmenu = MenuV:CreateMenu(false, "FiveEye ESP Menu", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: ESP Menu')
...
local espdistancia = espmenu:AddRange({ icon = '🔐', disabled = soloadmin, label = 'ESP Range', min = 1, max = 10, value = 1, description = 'Configura la distancia a donde puede alcanzar el ESP \n( 10 = Alcance en todo el mapa )', saveOnUpdate = true })
...
local menu3 = MenuV:CreateMenu(false, "FiveEye: Connected Players", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Connected Players')
menu3:On('open', function(m)
    ...
    for k, v in pairs(Players) do
        ...
        local jugador = MenuV:CreateMenu(' ', ('[ %s ]'):format(v.name), 'centerright', 255, 0, 0,  'size-150', 'example')
        ...
        local jugador_screenshot = jugador:AddButton({ icon = '📷', label = 'Screenshot', value = apolonovanadaesto, description = ('Screenshot player with ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('FiveEye:server:menuscreenshot', v.id)
                PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
            end
            })
        ...
        if especteando then
            ...
            local jugador_kick = jugador:AddButton({ icon = '🙉', label = 'Spectate', value = apolonovanadadeestoasdasdas, description = ('Spectate ID %s, Name: %s '):format(v.id, v.name),
                select = function()
                    ...
                    Citizen.Wait(100)
                    SetEntityCoords(PlayerPedId(), vector3(v.pos[1], v.pos[2], v.pos[3] + 50))
                    Citizen.Wait(500)
                    SpectatePlayer(jugadorsel)
                end
            })
        ...
        local jugador_ir = jugador:AddButton({ icon = '🛫', label = 'Goto', value = apolonovanadaesto, description = ('Go to ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('FiveEye:server:menugoto', v.id)
                PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
            end
        })
        ...
        local jugador_traer = jugador:AddButton({ icon = '🛬', label = 'Bring', value = apolonovanadaesto, description = ('Bring ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('FiveEye:server:menubring', v.id)
            end
        })
        ...
        local jugador_revivir = jugador:AddButton({ icon = '🩹', label = 'Revive', value = apolonovanadaesto, description = ('Revive ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('terevivo:atimismo', v.id)
                PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
            end
        })
        ...
        local jugador_kick = jugador:AddButton({ icon = '🦶', label = 'Kick', value = apolonovanadaesto, description = ('Kick ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('FiveEye:server:menukick', v.id, "Reason not Specified")
            end
        })
        ...
        local jugador_banear = jugador:AddButton({ icon = '🚷', disabled = soloadmin, label = 'Ban', value = apolonovanadaesto, description = ('Banear ID %s, Name: %s '):format(v.id, v.name),
            select = function()
                TriggerServerEvent('FiveEye:server:menuban', v.id, "Reason not Specified")
            end
        })
...
local menu4 = MenuV:CreateMenu(false, "FiveEye Server Tools", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Server Tools')
...
menu4_deletevehicles:On('select', function(item)
    TriggerServerEvent("FiveEye:ClearVehs")
end)

menu4_deleteprops:On('select', function(item)
    TriggerServerEvent("FiveEye:ClearProps")
end)

menu4_deletepeds:On('select', function(item)
    TriggerServerEvent("FiveEye:ClearPeds")
end)

-- MENU 5
local menu5 = MenuV:CreateMenu(false, "FiveEye Teleport Options", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Teleport Options')
...
-- MENU 6
local menu6 = MenuV:CreateMenu(false, "FiveEye Vehicle Tools", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Vehicle Tools')
...
-- MENU 7
local menu7 = MenuV:CreateMenu(false, "FiveEye Admin Tools", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: TP Tools Tools')
local menu7_acinfo = menu7:AddButton({ icon = 'ℹ️', label = 'AntiCheat Version: 1.0', description = 'discord.gg/p4vgQjyzQ8' })
local menu7_creatorac = menu7:AddButton({ icon = 'ℹ️', label = 'FiveEye by Nacho, Apolo & VisiBait', description = 'discord.gg/p4vgQjyzQ8' })

-- NEW MENUS (Augusto/Mopped7)

-- Menu 8 (Skin do PED)
local menu8 = MenuV:CreateMenu(false, "Change PED Skin", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'Change Characters skin')
...
local menu9 = MenuV:CreateMenu(false, "Vision Functions", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'Activate/Deactivate Vision Functions')
...
-- PRINCIPAL MENU
local menu = MenuV:CreateMenu(false, "Welcome to FiveEye's Admin Menu", 'centerright', 255, 0, 0, 'size-150', 'example', 'menuv', 'FiveEye: Main Menu')
...
RegisterCommand('fiveeyemenu', function()
    TriggerServerEvent('FiveEye:reqmenu')
end, false)
